...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function fetchAddress(block, borough, lot) {
    const url = `/api/data?borough=${borough}&block=${block}&lot=${lot}`;
  ...
  // Google Sheets Export (WIP | Some example code I had Chat GPT whip up temporarily)
  ...
  function exportToGoogleSheets(data, spreadsheetId) {
    gapi.client.sheets.spreadsheets.values.append({
      ...
      window.alert('Data exported to Google Sheets!');
    }, function(reason) {
  ...
  loadGoogleAPIs(function() {
    authenticateUser(function() {
      openPicker(function(data) {
        ...
        exportToGoogleSheets(dataToExport, spreadsheetId);
      });
    });
  });
*/
